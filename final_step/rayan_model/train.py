...
Architecture:
  AWD-LSTM pretrained via fastai/ULMFiT.
  Classification head: concat-pooling -> BatchNorm -> Dropout -> Linear -> ReLU
  -> BatchNorm -> Dropout -> Linear(5).

3 Ablation Runs:
  Run 1: Frozen AWD-LSTM encoder, only classifier head trained
  Run 2: Frozen encoder + EDA augmentation on minority classes
  Run 3: Frozen -> gradual full unfreeze + EDA + discriminative LRs

Outputs saved to rayan_model/diagrams/
"""
...
# Gradual-unfreeze phase (Run 3 only)
EPOCHS_UNFREEZE = 8
LR_UNFREEZE     = slice(1e-5, 1e-3)
...
print("Model  : ULMFiT AWD-LSTM (pretrained)")
...
def eda(sentence, alpha=0.1, num_aug=1):
    ...
    for _ in range(num_aug):
        ...
        if op == "sr":
            ...
            aug = random_insertion(words, n)
        elif op == "rs":
            aug = random_swap(words, n)
        else:
            aug = random_deletion(words, alpha)
...
class EpochLossTracker(Callback):
    ...
    def after_batch(self):
        if self.training:
            ...
            if loss is not None:
                self._batch_buf.append(float(loss.detach().cpu()))

    def after_epoch(self):
        if len(self._batch_buf) > 0:
...
def get_predictions(learn, texts):
    dl    = learn.dls.test_dl(texts, bs=BS, shuffle=False)
    probs, _ = learn.get_preds(dl=dl, reorder=False)
    vocab = list(learn.dls.vocab[1])
...
def run_experiment(run_name, train_df, val_df, test_df,
    ...
    train_losses.extend(tracker.epoch_train_losses)
    val_losses.extend([float(v[0]) for v in learn.recorder.values[-EPOCHS_FROZEN:]])
    ...
    if gradual_unfreeze:
        ...
        train_losses.extend(tracker.epoch_train_losses)
        val_losses.extend([float(v[0]) for v in learn.recorder.values[-EPOCHS_UNFREEZE:]])
    ...
    misclassified = [
        ...
        for i in range(len(ts_lbl_str))
        if ts_lbl_str[i] != ts_prd_str[i]
    ]
